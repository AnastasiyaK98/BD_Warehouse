...
<body style="margin: 0">
  <content>
    ...
    <okno>
      ...
      <br>
      <br>
      <form action="http://localhost/domains/cars.php?name=CARS&tab=МАШИНЫ" method="POST" style="position: relative; width: 300px; height: 40px; margin: 10px auto; right:525px">
      ...
      <?php if (isset($_GET['del'])) {
        $id = $_GET['del'];
        $sql = "DELETE FROM $_REQUEST[name] WHERE IDM=$id";
      ...
      if (isset($_POST['dob'])) {
        $conn = sqlsrv_connect($serverName, $connectionInfo);      
        $marka = $_POST['Marka'];
        $fi= $_POST['FIVladelets'];
        $gruz = $_POST['LoadCapacityTons'];
        $status = $_POST['St_'];
        $sql = "INSERT INTO $_REQUEST[name] (Marka, FIVladelets, LoadCapacityTons, Status) VALUES ('{$marka}','{$fi}' ,'{$gruz}','{$status}')";
        if (sqlsrv_query($conn, $sql)) { ?>
          <div class="alert" style="background-color: #89AC76">
            <span class="closebtn" onclick="this.parentElement.style.display='none';">×</span>
      ...
      <div class="scroll-table">
        ...
        <div class="scroll-table-body">
          <table>
            ...
            <tbody>
              <!-- Если выполнен поиск -->
              <?php if (isset($_POST['poisk'])) {
                /*Поиск по всем столбцам*/                        
                $sql = "SELECT * FROM $_REQUEST[name] WHERE IDM LIKE '%$_POST[poisk]%' OR Marka LIKE '%$_POST[poisk]%'
                OR FIVladelets LIKE '%$_POST[poisk]%'OR LoadCapacityTons LIKE '%$_POST[poisk]%' OR LoadCapacityTons LIKE '%$_POST[poisk]%'
                OR Status LIKE '%$_POST[poisk]%' ";
                ...
                if ($result != null) {
                  ...
                  while ($row = sqlsrv_fetch_array($result,  SQLSRV_FETCH_ASSOC)) {
                    ...
                    print '<td>';
                    print '<a href="http://localhost/domains/cars.php?name=CARS&tab=Машины&del=' . $row['IDM'] . '"><img src="icons8-удалить-20.png" title="Удалить"></a>';
                    print("&nbsp &nbsp &nbsp");
                    print '<a href="http://localhost/domains/cars.php?name=CARS&tab=Машины&red=' . $row['IDM'] . '" onclick="openForm(); return false;"><img src="icons8-редактировать-20.png" title="Редактировать"></a>';
                    print '</td>';
                    print '</tr>';
                  }
                }
                ...
                if ($result != null) {
                  ...
                  while ($row = sqlsrv_fetch_array($result,  SQLSRV_FETCH_ASSOC)) {
                    ...
                    print '<a href="http://localhost/domains/cars.php?name=CARS&tab=Машины&del=' . $row['IDM'] . '"><img src="icons8-удалить-20.png" title="Удалить"></a>';
                    print("&nbsp &nbsp &nbsp");
                    print '<a  href="http://localhost/domains/cars.php?name=CARS&tab=Машины&red=' . $row['IDM'] . '" onmouseout="openForm(); return false""><img src="icons8-редактировать-20.png" title="Редактировать"></a>';
                    print '</td>';
                    print '</tr>';
                  }
                }
      ...
      if (isset($_GET['red'])) {
        $id = $_GET['red'];
        $conn = sqlsrv_connect($serverName, $connectionInfo);
        $sql = "SELECT * FROM $_REQUEST[name] WHERE IDM=$id";
        $result = sqlsrv_query($conn, $sql);
        $row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC);
        $marka = $row['Marka'];
        $fi= $row['FIVladelets'];
        $gruz = $row['LoadCapacityTons'];
        $status = $row['Status'];
      ?>  
        <!-- Форма для редактирования -->
        <div class="form-popup" id="myForm_">
          <form action="" method="post" class="form-container">         
            <label for="Marka_"><b>Марка</b></label>
            <input type="text" value="<?php echo ($marka) ?>" name="Marka_" required>
            <label for="FIVladelets_"><b>ФИ Владельца (руб)</b></label>
            <input type="text" name="FIVladelets_" value="<?php echo ($fi) ?>" required>
            <label for="LoadCapacityTons_"><b>Грузоподъёмность (тонны)</b></label>
            <input type="text" value="<?php echo ($gruz) ?>" name="LoadCapacityTons_" required>
            <label for="St_"><b>Статус</b></label>
          <select name="St_" id="myForm_">
          <option value="<?php echo ($status) ?>" selected><?php echo ($status) ?></option>
            <option value="На вызове">На вызове</option>
      ...
      if (isset($_POST['soxr'])) {
        ...
        $conn = sqlsrv_connect($serverName, $connectionInfo);
        $marka = $_POST['Marka_'];
        $fi= $_POST['FIVladelets_'];
        $gruz = $_POST['LoadCapacityTons_'];
        $status = $_POST['St_']; 
        $sql = "UPDATE $_REQUEST[name] SET Marka='{$marka}', FIVladelets='{$fi}', LoadCapacityTons='{$gruz}', Status='{$status}' WHERE IDM=$id ";
      ...
      <div class="form-popup" id="myForm">
        <form action="" method="post" class="form-container"id="myForm">
          ...
          <input type="text" placeholder="Пример: Камаз 65117" name="Marka" required>
          <label for="FIVladelets"><b>ФИ Владельца</b></label>
          <input type="text" placeholder="Пример: Кирилов Иван" name="FIVladelets" required>
          <label for="LoadCapacityTons"><b>Грузоподъёмность (тонны)</b></label>
          <input type="text" placeholder="Пример: 14" name="LoadCapacityTons" required>
          <label for="St_"><b>Статус</b></label>
          <select name="St_" id="myForm">
            <option value="На вызове">На вызове</option>
            <option value="Свободен">Свободен</option>
        </select>
        <br><br>               
